...
LastEditTime: 2023-05-04 16:52:32
...
def note_color_threshold_for_white_bkg(img,sat_remap_order=0.5,dilate_rad = 0,Thresh_bias = 50, Thresh_dia=351,mode = 0):
    ## pre processing
    grey = cvtColor(img,COLOR_RGB2GRAY)
    thresh = adaptiveThreshold(grey,255,ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY,Thresh_dia,Thresh_bias)
    ...
    if mode == 0:#Black & White
        ...
        saturated_HSV[:,:,1] = power(saturated_HSV[:,:,1]/255,sat_remap_order)*255
        ...
        thresh_RGB[saturated_HSV[:,:,1]>100] = img_sat_remap[saturated_HSV[:,:,1]>100]
        return thresh_RGB    
    
def note_color_threshold_for_black_bkg(img,sat_remap_order=0.5,dilate_rad = 0,Thresh_bias = 50, Thresh_dia=351,mode = 0):
    ## pre processing
    img = bitwise_not(img)
    grey = cvtColor(img,COLOR_RGB2GRAY)
    thresh = adaptiveThreshold(grey,255,ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY,Thresh_dia,Thresh_bias)
...
def note_color_threshold(img,sat_remap_order=0.5,dilate_rad = 0,invert_threshold=127):
    ## pre processing
    # (considering backgroud color, for cases like blacboard notes)
    if mean(img)>invert_threshold:#似乎需要更换更好的算法处理黑白 因为黑板亮度不好说（用直方图）
        img = bitwise_not(img)
    grey = cvtColor(img,COLOR_RGB2GRAY)
    thresh = adaptiveThreshold(grey,255,ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY,351,50)
...
class BlackboardRecorder(object):
    ...
    def __init__(self, src_img):
        ...
        # Thresh
        thresh_rad = int(self.thresh_rad_rel*min(self.img.shape[:1])/2)*2+1 # must be odd
        self.grey = cvtColor(self.img,COLOR_RGB2GRAY)
        # for White Bkg
        self.thresh = adaptiveThreshold(self.grey,255,ADAPTIVE_THRESH_MEAN_C,THRESH_BINARY,
    ...
    def find_possible_ROIs(self,thresh):
        ...
        '''        
        num_labels, labels, stats, centroids = connectedComponentsWithStats(thresh, connectivity=4, ltype=None)
        ...
        res = cv2.drawContours(draw_img, [approx], -1, (255, 0, 0), int(draw_img.shape[0]/20))
        if self.graph_display:
            subplot(1,4,3)
            imshow(res) 
